...
be1=(ga1-rho*ga2)/r2
be2=(ga2-rho*ga1)/r2
bpdf=2*CopulaModel::dbvn2(y1,y2,rho)*pnorm((be1*y1+be2*y2)/s)
bpdf
}
...
be1=(ga1-rho*ga2)/r2
be2=(ga2-rho*ga1)/r2
y1=sn::qsn(u,0,1,alpha=a1)
y2=sn::qsn(v,0,1,alpha=a2)
bpdf=2*CopulaModel::dbvn2(y1,y2,rho)*pnorm((be1*y1+be2*y2)/s)
denom=sn::dsn(y1,alpha=a1)*sn::dsn(y2,alpha=a2)
cpdf=bpdf/denom
cpdf
}
...
a1=ga1/sqrt(1-ga1^2)
a2=ga2/sqrt(1-ga2^2)
y1=sn::qsn(u,alpha=a1); y2=sn::qsn(v,alpha=a2)
cdf=sn::pmsn(c(y1,y2),Omega=rmat,alpha=alpv)
cdf
}
...
ccdf=sn::psn(y2,xi=xi21,omega=om21,alpha=a21,tau=tau21)
ccdf
}
...
ccdf=sn::psn(y1,xi=xi12,omega=om12,alpha=a12,tau=tau12)
ccdf
}
...
a1=ga1/sqrt(1-ga1^2)
a2=ga2/sqrt(1-ga2^2)
y1=sn::qsn(u,alpha=a1); #y2=qsn(v,alpha=a2)
# parameters on cond cdf to match univariate extended skew-normal
om21=sqrt(1-rho^2)
s=1-ga1^2-ga2^2-rho^2+2*ga1*ga2*rho # should be positive
s=sqrt(s)/om21
...
xi21=rho*y1
a21=(ga2-rho*ga1)/s/om21
y2=sn::qsn(p,xi=xi21,omega=om21,alpha=a21,tau=tau21)
v=sn::psn(y2,alpha=a2)
v
}
...
xi12=rho*y2
a12=(ga1-rho*ga2)/s/om12
y1=sn::qsn(p,xi=xi12,omega=om12,alpha=a12,tau=tau12)
u=sn::psn(y1,alpha=a1)
u
}
